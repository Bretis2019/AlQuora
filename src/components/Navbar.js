...
export default function Navbar(props){
    ...
    return(
        <div>
            <div className={"fixed min-h-[60px] bg-gray-100"}>
                <div className={"px-[15%] flex align-center justify-between w-screen min-h-[50px] bg-white py-1 pt-2 shadow-nav"}>
                    <h1 onClick={() => props.pageChange("home")} className={"text-red-500 text-3xl font-extrabold font-logo cursor-pointer"}>Quora</h1>
                    ...
                    <input  onChange={e => props.handleChange(e.target.value)} className={"rounded-md w-96 px-2 max-h-9 shadow-nav"} type={"text"} placeholder={"Search..."}/>
                    <div className={"rounded-full bg-gradient-to-r from-cyan-500 to-blue-500 w-[2.5rem]"}></div>
                    <button className={"rounded-2xl bg-red-500 text-white px-8 py-1 flex justify-center items-center gap-x-4"} onClick={props.handleClickShow}>Add a question<Arrow /></button>
                </div>
            </div>
        </div>
    )
